    ...
    const SL_t *sl, bool_t prev_only)
{
	...
	for (int i = 0; i < NUM_RA_INFOS; i++) {
		...
		if ((initial && !ri->initial) || (!initial && !ri->subseq)) {
			dbg_log(ra, 4, "CULL init/subseq " PRINTF_RI_FMT,
			    PRINTF_RI_ARGS(ri));
			continue;
		}
